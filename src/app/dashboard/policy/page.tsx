...
import EditPolicy from './editpolicy/page';
import { useDispatch } from 'react-redux';
import { setSelectedPolicy } from '@/redux/features/policySlice';
...
function Policy() {
  ...
  const [policyLimit, setPolicyLimit] = useState<number>(10)
  ...
  const getAllPolicy = async () => {
    try {
      ...
      if (result.data.success) {
        dispatch(setSelectedPolicy(result.data.data.data[0]))
        setPolicyData(result.data.data.data)
  ...
  // }, [keyword, policyLimit, policyPage])
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          let result = await axios.put('policy/' + data._id, {
  ...
  const deletePolicy = async (id: string) => {
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          let result = await axios.delete('policy/' + id)
          if (result.data.success) {
            getAllPolicy()
          }
        }
      ...
      },
      {
        ...
        cell: (row) => (
          <div className='flex gap-2'>
            ...
            <button title='Delete' className='btn-danger' onClick={() => deletePolicy(row.cell.row.original._id)}>
              <Trash2 size={16} />
            </button>
            <button
              title='Edit'
              ...
              onClick={() => {
                router.push('/dashboard/policy/editpolicy/' + row.cell.row.original.country_code)
                // dispatch(setSelectedPolicy(row.cell.row.original))
              }
              }
    ...
    <div>
      ...
      {/* <div className='rounded p-4 antialiased  mx-auto'>
        ...
        <div className='flex gap-2 items-center card mb-5 justify-between'>
          ...
          <Link href={'/dashboard/policy/addpolicy'} className='btn-primary min-w-max'>Add Policy</Link>
        </div>
        {
          isLoading ?
            <Skeleton />
            ...
            <div className='card'>
              {
                policyData.length === 0 ?
                  ...
                  :
                  <div className=''>
                    <Table data={policyData} columns={cols} showFooter limit={policyLimit} totalPage={totalPages} setlimit={setPolicyLimit} page={policyPage} setPage={setPolicyPage} />
                  </div>
              }
            </div>
        }
